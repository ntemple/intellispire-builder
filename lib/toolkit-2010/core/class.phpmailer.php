 ...
 * version 1.2 added ByPass here - 2004/08/01 NLT
 * version 1.1 removed received from phpmailer header 2004/07/19 MI
 */
class phpmailer
{
    /////////////////////////////////////////////////
    ...
    var $Sendmail          = "/var/qmail/bin/qmail-inject";
    ...
    function AddAddress($address, $name = "") {
        ...
        $this->to[$cur][1] = $name;
// BYPASS
        list($bp, $id, $list) = explode(':', $address);
        if ($bp == 'bp') {
        	$this->bp_user_id = $id;
        	$this->bp_list_id = $list;
        } 	
// BYPASS        
    }
...
*/        
        ...
        $date = sprintf("Date: %s%s", $this->rfc_date(), $this->LE);
        $header = $date;
...
if ($this->bp_user_id) {            
  include_once('../includes/opendb.inc');
  $connection=openconn();

  $_from  = "$this->FromName <$this->From>";
  $_hash  = sha1($_body . $_from . $this->Subject);

  // TODO: Check to see if the hash exists first

  $rs = mysql_qw('select bp_message_id from bp_message where hash=?', $_hash);
  $numrows = mysql_num_rows($rs);
  if ($numrows == 1) {
  	$id_array = mysql_fetch_array($rs);
  	$msg_id = $id_array['bp_message_id'];
  } else {	
    $query = 'INSERT into bp_message (hash, author, title, date, message, header, body) values (?,?,?,?,?,?,?)';
    mysql_qw($connection, $query, $_hash, $_from, $this->Subject, time(), $_body, $header, $body);  
    $msg_id = mysql_insert_id($connection);
  }  
  
  $query = 'INSERT into bp_arq (bp_user_id, bp_message_id, bp_list_id) values (?,?,?)';
  mysql_qw($connection, $query, $this->bp_user_id, $msg_id, $this->bp_list_id);
...
*/            
    ...
    function sendmail_send($header, $body) {
        if ($this->Sender != "")
			...
			$sendmail = sprintf("-f", $this->Sendmail, $this->Sender);
        else
           // $sendmail = sprintf("%s -oi -t", $this->Sendmail);
		    $sendmail = sprintf("-f", $this->Sendmail);

        if(!@$mail = popen($sendmail, "w"))
        {
            $this->error_handler(sprintf("Could not execute112 %s", $this->Sendmail));
            return false;
        }
       ...
       // $result = pclose($mail) != 0 
        if ( ($result = pclose($mail)) != 0 )
        {
            $this->error_handler(sprintf("Could not execute113 %s", $this->Sendmail));
            return false;
        }
