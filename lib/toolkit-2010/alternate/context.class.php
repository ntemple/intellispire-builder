...
# This opens up the system, and allows attacks. Careful!
  ...
  function output($tplfile = '') {
       ...
       $template = new HTML_Template_Flexy_Ex($this->opt);
  ...
  # To be called ONLY from the template
  function set($name, $value, $persist = false) {
      $this->$name = stripslashes($value);  // Is this OK for all cases?
...
class HTML_Template_Flexy_Ex extends HTML_Template_Flexy {
	function raiseError($msg) {
		error_log($msg);
		if (strpos($msg, 'Could not find Template') === 0) {
			return NULL;		
		}
/*
		print $msg;
